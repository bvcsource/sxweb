...
class Zend_View_Helper_CanWriteToVolume extends Zend_View_Helper_Abstract {
	...
	public function canWriteToVolume() {
        if (!Zend_Auth::getInstance()->hasIdentity()) {
            return FALSE;
        }
        
		$whoami = Zend_Auth::getInstance()->getIdentity()->getPreferences()->get('whoami');
		if (is_null($whoami)) {
			return FALSE;
		}
		if (isset($this->view->acl)) {
			if (!empty($this->view->acl) && is_array($this->view->acl)) {
				foreach($this->view->acl as $user_acl) {
					if ($user_acl['user'] == $whoami) {
						return in_array('write', $user_acl['perms']);
					}
				}
			}
		}
