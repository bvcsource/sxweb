...
class My_Utils {
    ...
    public static function getRandomBytes($length) {
        $bytes = openssl_random_pseudo_bytes($length, $crypto);
        if ($crypto === FALSE) {
            Zend_Controller_Front::getInstance()->getParam('bootstrap')->getResource('log')->err(__METHOD__ . ': OPENSSL FAILED TO GENERATE RANDOM BYTES!');
        }
        return $bytes;
    }

    /**
     * Test if the pseudo-random string of bytes generated by {@link getRandomBytes} 
     * are safe to use. 
