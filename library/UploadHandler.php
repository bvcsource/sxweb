 ...
 * IMPORTANT:
 *
 * this class was modified to send files to the SX server.
 * You MUST pass the options:
 * 'sx_volume_param' - string
 * 'sx_path_param' - string
 *
 * They are the names of the parameters into the Zend Request object containing
 * the volume and the path on the SX cluster to save the file to.
 * @see validate
 * @see store_on_sx_cluster
 */
class UploadHandler {
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            'sxurl' => '', // The destination path on the server
            'sx_volume_param' => '', // The HTTP parameter that tells the volume on SX server
            'sx_path_param' => '', // The HTTP parameter that tells the path on SX server
    ...
    protected function store_on_sx_cluster($file) {
        try {
            ...
            $file->error = 'Internal error, upload failed.';
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
        // Check if we can use the SX server
        $logger = Zend_Controller_Front::getInstance()->getParam('bootstrap')->getResource('log');

        if (!Zend_Auth::getInstance()->hasIdentity()) {
            $file->error = 'Authentication failed, please login.';
            return FALSE;
        }

        // Check SX server parameters
        if (empty($this->options['sx_volume_param']) || empty($this->options['sx_path_param'])) {
            $file->error = 'Internal error, badly configured script.';
            return FALSE;
        }
        ...
        $logger->debug(__METHOD__.' Volume is: '.var_export($volume, TRUE).' Path is: '.var_export($path, TRUE));

        $the_path = My_Utils::slashPath($volume).My_Utils::removeSlashes($path, TRUE);
        ...
        if ($post_max_size && ($content_length > $post_max_size)) {
            ...
            $file->error = $this->get_error_message('File size is too big, maximum allowed size is: '.($post_max_size / (1024 * 1024)).'MB');
            return false;
        }
    ...
    protected function get_file_name($file_path, $name, $size, $type, $error,$index, $content_range) {
        return $this->get_unique_filename($file_path,$this->trim_file_name($file_path, $name, $size, $type, $error, $index, $content_range),
            $size, $type, $error,
            $index, $content_range
        );
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,  $index = null, $content_range = null) {
        ...
        $file->name = md5($this->get_file_name($uploaded_file, $name, $size, $type, $error,  $index, $content_range).time());
    ...
    public function post($print_response = true) {
        ...
        $file_name = $this->get_server_var('HTTP_CONTENT_DISPOSITION') ?
            rawurldecode(preg_replace('/(^[^"]+")|("$)/','',$this->get_server_var('HTTP_CONTENT_DISPOSITION'))) : null;
